...
void CheckComm(DMEM_AllData *dmem_all_data);
int CheckOutsideDoneFlag(DMEM_AllData *dmem_all_data);
...
void DMEM_Add(DMEM_AllData *dmem_all_data)
{
  ...
  // return;
   ...
   if (dmem_all_data->input.res_compute_type == LOCAL_RES){
      DMEM_ResidualToGridk_LocalRes(dmem_all_data, F_array_fine[finest_level], F_array_gridk[finest_level]);
      HYPRE_Real *b_local_data_gridk = hypre_VectorData(hypre_ParVectorLocalVector(dmem_all_data->vector_gridk.b));
      HYPRE_Real *f_local_data_gridk = hypre_VectorData(hypre_ParVectorLocalVector(F_array_gridk[finest_level]));
      for (int i = 0; i < num_rows_gridk; i++){
         b_local_data_gridk[i] = f_local_data_gridk[i];
      }
   }
   ...
   if (my_grid == num_levels-1){
      ...
      if (dmem_all_data->input.solver == BPX){
         ...
         hypre_ParCSRMatrixMatvec(1.0,
                                  ...
                                  0.0,
                                  U_array[fine_level]);
         f_local_data = hypre_VectorData(hypre_ParVectorLocalVector(F_array[fine_level]));
         u_local_data = hypre_VectorData(hypre_ParVectorLocalVector(U_array[fine_level]));
         A_data = hypre_CSRMatrixData(hypre_ParCSRMatrixDiag(A_array[fine_level]));
         A_i = hypre_CSRMatrixI(hypre_ParCSRMatrixDiag(A_array[fine_level]));
         num_rows = hypre_CSRMatrixNumRows(hypre_ParCSRMatrixDiag(A_array[fine_level]));
         hypre_ParCSRMatrixMatvec(1.0,
                                  A_array[fine_level],
                                  U_array[fine_level],
                                  0.0,
                                  Vtemp);
         for (int i = 0; i < num_rows; i++){
            u_local_data[i] = dmem_all_data->input.smooth_weight * (f_local_data[i] - v_local_data[i]) / A_data[A_i[i]];
         }
         coarsest_level -= 1;
      }
      else {
...
{
   if (dmem_all_data->input.res_compute_type == GLOBAL_RES){
      if (CheckMessageFlagsNotValue(dmem_all_data, &(dmem_all_data->comm.finestToGridk_Correct_outsideSend), 0) == 1 &&
          CheckMessageFlagsNotValue(dmem_all_data, &(dmem_all_data->comm.finestToGridk_Correct_outsideRecv), 0) == 1 &&
          CheckMessageFlagsNotValue(dmem_all_data, &(dmem_all_data->comm.finestToGridk_Residual_outsideSend), 0) == 1 &&
          CheckMessageFlagsNotValue(dmem_all_data, &(dmem_all_data->comm.finestToGridk_Residual_outsideRecv), 0) == 1 &&
          CheckMessageFlagsNotValue(dmem_all_data, &(dmem_all_data->comm.finestIntra_outsideSend), 0) == 1 &&
          CheckMessageFlagsNotValue(dmem_all_data, &(dmem_all_data->comm.finestIntra_outsideRecv), 0) == 1){
...
void AsyncRecvCleanup(DMEM_AllData *dmem_all_data)
{
  ...
  // PrintMessageFlags(dmem_all_data, &(dmem_all_data->comm.gridjToGridk_Correct_outsideSend));
   ...
   while (1){
      int break_flag = 0;
      if (dmem_all_data->input.res_compute_type == GLOBAL_RES){
         if (CheckMessageFlagsValue(dmem_all_data, &(dmem_all_data->comm.finestToGridk_Correct_outsideRecv), 2) == 1 &&
             CheckMessageFlagsValue(dmem_all_data, &(dmem_all_data->comm.finestToGridk_Residual_outsideRecv), 2) == 1 &&
             CheckMessageFlagsValue(dmem_all_data, &(dmem_all_data->comm.finestIntra_outsideRecv), 2) == 1){
            break;
         }
      }
      else {
...
int CheckConverge(DMEM_AllData *dmem_all_data)
{
   ...
   if (dmem_all_data->input.async_flag == 1){
      if (dmem_all_data->input.converge_test_type == GLOBAL_CONVERGE){
         int my_comm_done;
         CheckOutsideDoneFlag(dmem_all_data);
         begin = MPI_Wtime();
