...
void SMEM_Sync_AFACx_Vcycle(AllData *all_data)
{
   #pragma omp parallel
   {
      ...
      for (int i = 0; i < all_data->thread.thread_levels[my_id].size(); i++){
        ...
        //               all_data->pardiso.info.iparm,
        //               &(all_data->pardiso.info.msglvl),
        //               all_data->level_vector[thread_level].r[this_grid],
        //               all_data->level_vector[thread_level].u_fine[this_grid],
        //               &(all_data->pardiso.info.error));
        //    }
        // }
        // else {
        ...
        // this_grid = thread_level;
        // ns = all_data->thread.A_ns[this_grid][my_id];
        // ne = all_data->thread.A_ne[this_grid][my_id];
        // for (int i = ns; i < ne; i++){
        //    all_data->level_vector[thread_level].e[this_grid][i] =
        //       all_data->level_vector[thread_level].u_fine[this_grid][i];
        // }
        // SMEM_LevelBarrier(all_data, thread_level);
        // if (thread_level > 0){
