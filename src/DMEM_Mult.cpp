...
void DMEM_Mult(DMEM_AllData *dmem_all_data)
{
   ...
   for (dmem_all_data->iter.cycle = 0; dmem_all_data->iter.cycle < dmem_all_data->input.num_cycles; dmem_all_data->iter.cycle += 1){
	       ...
	       HYPRE_Int cycle)
{
   ...
   for (HYPRE_Int level = 0; level < coarsest_level; level++){
      ...
      begin = MPI_Wtime();
      if (level == 0){
         hypre_ParVectorCopy(dmem_all_data->vector_fine.r, Vtemp);
        // hypre_ParVectorCopy(dmem_all_data->vector_fine.b, Vtemp);
        // hypre_ParCSRMatrixMatvec(-1.0,
        //                          A_array[fine_grid],
        //                          U_array[0],
        //                          1.0,
