...
for n in ${n_list[@]}
do
	./Main -format_output -num_runs ${num_runs} -n ${n} -num_cycles ${num_cycles} -num_threads ${num_threads} -solver ${solver} -problem ${problem} -async_type ${async_type} -converge_test_type all | tee -a ${file_name}
done
